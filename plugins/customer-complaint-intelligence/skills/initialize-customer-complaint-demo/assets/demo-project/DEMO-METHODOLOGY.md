...
- The email's customer reference, subject, and timestamp are enough to trace later evidence and action. Connector IDs and invented links stay out of the handoff.
...
- Live connectors change details. Gmail sends every synthetic message from the connected account, so customer identity is carried by the reference in the body and joined only during analysis. Sending to a fictional `.example` address created bounce notices; sending the held-out fixture to Bobby's own address avoided that noise.
- Connector efficiency is part of demo reliability. The live rehearsal showed that paginated ID search followed by 20-message body batches avoids Gmail's per-user concurrency failures; only failed IDs need one smaller retry.
- Gmail retrieval takes seconds; interpreting the full population is the substantive AI work and can take several minutes. Treat that first step as visible background heavy lifting rather than promising an instant result.
- The analytical reveal should shape the visualization. A focused overview-to-cohort-to-source path made the connected story clearer than a generic dashboard with many possible dimensions.
...
- Rehearsing a demo leaves residue across several surfaces: generated files, modified external-system state, action receipts, and tasks filled with testing history. A working rehearsal can therefore leave behind an environment that is confusing or unsuitable for the next presenter even though the demo itself succeeded.
